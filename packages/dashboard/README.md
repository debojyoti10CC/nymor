...
- **"Try it yourself" click-tested end-to-end in a real browser with the Freighter extension**: connected a funded testnet wallet, paid $0.01 for `/xlm-price`, and got real CoinGecko-sourced XLM price data back. Getting here required three CORS fixes on `nymor-resources` (see its `index.ts` for details) — `nymor-resources` had never been called from a browser before, so none of this surfaced in curl/node testing. Notably `@x402/fetch` v2.23.0 sets `Access-Control-Expose-Headers` (a response-only header) as a *request* header on the signed retry, which browsers reject in preflight unless the server explicitly allows it — a real upstream library quirk, worked around server-side rather than patched.
